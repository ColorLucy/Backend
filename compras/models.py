...
class Pedido(models.Model):
    ...
    estado = models.CharField(
        max_length=10, choices=ESTADO_PEDIDO_CHOICES, default="recibido"
    )
...
class ProductoPedido(models.Model):
    ...
    pedido = models.ForeignKey(
        Pedido, related_name="productos", on_delete=models.CASCADE
    )
    ...
    def save(self, *args, **kwargs):
        if not self.id_productos_pedido:
            max_id = ProductoPedido.objects.aggregate(
                max_id=Max("id_productos_pedido")
            )["max_id"]
